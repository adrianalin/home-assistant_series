...
from const import (
    ...
    SUPPORT_VOLUME_STEP, CONF_HOST, CONF_PORT, CONF_NAME, CONF_PASSWORD, STATE_OFF,
...
def setup_platform(config=None):
    ...
    if config is None:
        config = {}
    host = config.get(CONF_HOST, Config.host)
    port = config.get(CONF_PORT, Config.port)
    name = config.get(CONF_NAME, Config.name)
    password = config.get(CONF_PASSWORD, Config.password)
...
class MpdDevice:
    ...
    def update(self):
        ...
        list_playlists = self._client.listplaylists()
        print(list_playlists)
        curren_playlist = self._client.playlist()
        print(curren_playlist)
